...
void uart_on(void)
{
	...
	TXSTAbits.BRGH = 1;
#if 0
	/* PIC18F4431で9600baudを設定し、連続受信すると
	   STOPビットを認識しなくなる */
	BAUDCTLbits.BRG16 = 1;
	SPBRG = (_XTAL_FREQ/UART_BAUD_RATE+2)/4 - 1;
	SPBRGH = ((_XTAL_FREQ/UART_BAUD_RATE+2)/4 - 1)>>8;
#else
	BAUDCTLbits.BRG16 = 0;
	SPBRG = (_XTAL_FREQ/UART_BAUD_RATE+8)/16 - 1;
	SPBRGH = ((_XTAL_FREQ/UART_BAUD_RATE+8)/16 - 1)>>8;
#endif
...
{
	signed char r = (signed char) uart_tx_r - uart_tx_w - 1;
...
{
	signed char r = (signed char) uart_rx_w - uart_rx_r;
	if (r<0) r+=UART_BUF_SIZ;
	return (unsigned char) r;
}
